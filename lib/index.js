...
const resolvers = {
  ...
  Mutation: {
    ...
    createMessage: (_, { raw, userId, text }) =>
      (raw
        ? query(`INSERT INTO messages (userId, text) VALUES ('${userId}','${text}')`) // INSERT INTO `messages` (`id`,`text`,`createdAt`,`updatedAt`,`userId`) VALUES (DEFAULT,'doei pro','2018-01-18 13:10:50','2018-01-18 13:10:50',1);
        : User.update({ lastMessage: Date.now() }, { where: { id: userId } })
          .then((result) => {
            if (result[0]) {
              //= ==1 kan niet als je zoveel doet op zelfde userid. ik moet nog een random user functie maken in de artillery
