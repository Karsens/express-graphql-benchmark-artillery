...
const sequelize = new Sequelize(DATABASE, 'root', DING, {
  host: 'localhost',
...
const resolvers = {
  Query: {
    users: (_, { auth }) => User.findAll({ limit: 100 }).then(res=>res).catch(e=>console.log(e)),

    user: (_, { id }) => User.findOne({ where: { id } }).then(res=>res).catch(e=>console.log(e)),

    messages: (_, { userId }) =>
      Message.findAll({ where: { userId }, include: [User], limit: 100 }).then(res=>res).catch(e=>console.log(e)),
